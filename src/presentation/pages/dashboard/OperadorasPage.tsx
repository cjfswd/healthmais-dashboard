...
export function OperadorasPage() {
  const { categoryAggregation, pieAggregation, filteredData } = useDashboard();
  const { label: period, isFullYear } = usePeriodLabel();
  const fmt = (val: number) =>
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 }).format(val);

  const data = categoryAggregation('operadora');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <OperadoraChart data={data} currencyFormatter={fmt} />
        </div>
        <PieDataChart data={pieAggregation('operadora')} title="Distribuição por Operadora" />
      </div>

      <Card className="border border-zinc-200 rounded-lg shadow-sm overflow-hidden">
        <DashboardCardHeader icon={Building2} title="Detalhamento por Operadora" period={period} isFullYear={isFullYear} />
